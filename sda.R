...
# function makes U %*% L work by replacing na values with 0
fun <- function(x, y){
  z <- x * y
  t <- ifelse(is.na(z), 0, z)
  colSums(t)
}

# reference for what delta F should be
F_1986 <- fun(U_list[[1]], L_list1[[1]]) %*% Y_list[[1]]
...
delta_Ft <- SDAt(U_list[[3]], U_list[[2]], 
                 L_list1[[3]], L_list1[[2]], 
                 Y_list[[3]], Y_list[[2]])
all.equal(F_soll8, delta_Ft[["delta_F"]])
...
all.equal(F_soll8, loop[[3]][["delta_F"]])
